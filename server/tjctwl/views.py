...
def weixinpay(request):
    ...
    apiUrl_makeOrder = "https://qr.chinaums.com/netpay-route-server/api/"
    notifyUrl = "http://172.27.49.240:8080/h5pay/notifyUrl.do"
    returnUrl = "http://tjyzzpay.cn:8003/#/pages/pay/pay_result"
    msgSrc = "WWW.TANGKYZZ.COM"
    msgType = "wx.unifiedOrder"
    tradeType = "MINI"
    print (request.POST['js_code'])
    subOpenId = _weixin_auth_code2session(request.POST['js_code'])
    requestTimestamp = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
    merOrderId = "6585"+time.strftime('%Y%m%d%H%M%S', time.localtime())+str(random.randint(1000000,9999999))
    #mid = "898201612345678"
    mid = "898120082200361" 
    tid = "01481722"
    instMid = "MINIDEFAULT"
    totalAmount = "1"
    md5key = "rFdsDArHmxiZ5kntXci655KMbh5DWGjcHYGxXizdW8NbZke3"
    ...
    #encoded = urllib.urlencode(return_json)
    headers = {'Content-Type':'application/json'}
    rep = requests.post(url=apiUrl_makeOrder, data=return_json, headers=headers)
    #reuturn_url = apiUrl_makeOrder+"?"+encoded
    #print("最后返回的url")
    #print(reuturn_url)
    return _generate_json_message(True, reuturn_url)
